...
export default function EthiopiaMap() {
  ...
  return (
    <div className="relative w-full aspect-video md:aspect-[16/10] bg-background/40 rounded-xl overflow-hidden glass-card p-4 border border-border flex flex-col">
      {/* Legend & Controls Overlay */}
      <div className="absolute top-4 left-4 flex flex-col gap-3 z-10">
        <div className="px-3 py-1.5 rounded-md bg-background/90 border border-border backdrop-blur-md flex items-center gap-2 w-fit">
        ...
        {/* Legend Key */}
        <div className="px-3 py-2 rounded-md bg-background/90 border border-border backdrop-blur-md flex flex-col gap-1.5 w-fit">
          <p className="text-[9px] uppercase tracking-wider font-semibold text-muted-foreground mb-1">Risk Scale</p>
          <div className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full bg-red-500" /><span className="text-[10px] font-medium">High Risk</span></div>
          <div className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full bg-yellow-500" /><span className="text-[10px] font-medium">Moderate</span></div>
      ...
      <div className="w-full h-full relative cursor-crosshair">
        ...
        >
          ...
          <line x1="0" y1="512" x2="1024" y2="512" className="stroke-primary/10 stroke-1" strokeDasharray="6 6" />
          <line x1="512" y1="0" x2="512" y2="1024" className="stroke-primary/10 stroke-1" strokeDasharray="6 6" />
          ...
          {REGIONS.map((region, i) => {
            ...
            return (
              ...
              >
                ...
                />
                {/* Inner Dot */}
                <circle cx={region.cx} cy={region.cy} r={isHovered ? 10 : 8} fill={color} className="drop-shadow-lg" />
                {/* Label */}
                <text
                  x={region.cx + 20}
                  y={region.cy + 5}
                  className="text-sm fill-foreground font-semibold tracking-wide"
        ...
        <AnimatePresence>
          {hoveredRegion && (
            ...
            >
              <div className="w-64 bg-background/95 backdrop-blur-xl border border-border p-4 rounded-xl shadow-2xl relative overflow-hidden">
                <div 
                ...
                <div className="flex justify-between items-start mb-2">
                  <h4 className="font-bold text-base">{hoveredRegion.name}</h4>
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  <div className="bg-muted/50 p-2 rounded-md">
                    <p className="text-[10px] uppercase font-semibold text-muted-foreground pb-0.5">Active Cases</p>
                    <p className="text-xl font-bold tabular-nums">{hoveredRegion.cases}</p>
                  </div>
                  <div className="bg-muted/50 p-2 rounded-md">
                    <p className="text-[10px] uppercase font-semibold text-muted-foreground pb-0.5">Risk Level</p>
